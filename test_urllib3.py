...
def run_server():
    uvicorn.run(app, host="127.0.0.1", port=8087, log_level="critical")
...
if __name__ == "__main__":
    ...
    embedder_urllib = OpenAICompatibleEmbedder(
        model="dummy",
        url="http://127.0.0.1:8087/v1/embeddings",
        dimensions=10,
        provider="ollama"
    )
    ...
    start = time.time()
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        futures = [executor.submit(embedder_urllib3.embed, ["hello world"] * 10) for _ in range(50)]
        concurrent.futures.wait(futures)
    print(f"urllib3 time (concurrent with pooling): {time.time() - start:.4f}s")
