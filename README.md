...
The default database is in memory for a lightweight, zero-setup local experience. Set `RAG_DATABASE_PATH` to use file-backed SQLite when data should survive process restarts.
...
uv run uvicorn lightweight_rag.api.main:app --reload
```
...
uv run uvicorn lightweight_rag.api.main:app --host 127.0.0.1 --port 8000
```
