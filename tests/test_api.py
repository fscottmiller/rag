...
def test_rest_rejects_cross_origin_mutations_in_trusted_proxy_mode(service):
    ...
    protected_service = RAGService(
        ...
        service.embedder,
        service.chunker,
        app_settings(auth_mode="trusted-proxy"),
    )
...
def test_rest_rejects_documents_over_configured_limit(service):
    limited = RAGService(
        SQLiteStore(), service.embedder, service.chunker, app_settings(max_document_bytes=8)
    )
...
def test_rest_accepts_document_content_at_exact_byte_limit(service, path):
    limit = 8
    limited = RAGService(
        SQLiteStore(), service.embedder, service.chunker, app_settings(max_document_bytes=limit)
    )
    client = TestClient(create_app(limited))
    content = "x" * limit
...
def test_rest_rejects_large_request_before_parsing(service):
    limited = RAGService(
        ...
        service.embedder,
        service.chunker,
        app_settings(max_document_bytes=8, max_request_bytes=32),
    )
    client = TestClient(create_app(limited))
    response = client.post(
        "/documents",
