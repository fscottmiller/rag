...
def run_server():
    uvicorn.run(app, host="127.0.0.1", port=8086, log_level="critical")
...
if __name__ == "__main__":
    ...
    class EmbedderHttpx(OpenAICompatibleEmbedder):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        futures = [executor.submit(embedder_urllib.embed, ["hello world"] * 10) for _ in range(50)]
        concurrent.futures.wait(futures)
    print(f"urllib time (concurrent): {time.time() - start:.4f}s")

    start = time.time()
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        futures = [executor.submit(embedder_httpx.embed, ["hello world"] * 10) for _ in range(50)]
        concurrent.futures.wait(futures)
    print(f"httpx time (concurrent with pooling): {time.time() - start:.4f}s")
