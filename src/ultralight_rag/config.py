...
class Settings:
    ...
    def __post_init__(self) -> None:
        provider_config = PROVIDER_DEFAULTS.get(
            canonical_provider(self.embedding_provider), PROVIDER_DEFAULTS["default"]
        )
        if self.embedding_model is None:
            object.__setattr__(self, "embedding_model", provider_config["model"])
        if self.embedding_url is None:
            object.__setattr__(self, "embedding_url", provider_config["url"])
    ...
    def from_env(cls) -> "Settings":
        configured_provider = os.getenv("RAG_EMBEDDING_PROVIDER")
        ...
        provider_config = PROVIDER_DEFAULTS.get(
            canonical_provider(provider), PROVIDER_DEFAULTS["default"]
        )
        if configured_provider is not None and not api_key and provider_config["uses_openai_key"]:
